...
| `table_files(ns, table)` | `sqe-catalog` | One row per data file in the current snapshot. Columns: `file_path`, `file_format`, `record_count`, `file_size_in_bytes`, `column_sizes`, `value_counts`, `null_value_counts`, `partition`, `lower_bounds`, `upper_bounds`. `lower_bounds` / `upper_bounds` are JSON maps of `{field_id: value}` and let you verify a compaction produced a prunable layout (after a sort compaction the output files carry disjoint ranges on the sort column). `iceberg_metadata_tvf.rs` | `t$files` | - | `t.files` |
