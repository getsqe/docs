...
| `a feature branch` | open MR | The actual runtime filter pushdown. Two layered commits described below. |
...
The fix: after an earlier change (`6a03124`) brought Path B-2 to SQE's own
...
| 5/1  | post an earlier change (Path B-2 wiring SSB) | 8,410 ms | SSB-only run, cold Trino |
...
A side branch (`a feature branch`, commit `f022619`)
...
`sqe-catalog/src/parquet_writer_config.rs`): users who ask for
