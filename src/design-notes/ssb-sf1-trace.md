...
> phase-level rows in `EXPLAIN ANALYZE` (an earlier change).
...
`sqe-bench` (an earlier change), we can see exactly where each query spends its
time.
...
`sqe-catalog/src/iceberg_scan.rs`) emits a `tracing::debug!`
...
`sqe-catalog/src/iceberg_scan.rs`) was missing the
...
The trace work itself (an earlier change, !122) is the foundation: every future
