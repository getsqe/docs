...
The store is `RangerStore` in `sqe-policy/src/ranger_store.rs`. The
rewriter is `PolicyPlanRewriter` in `sqe-policy/src/plan_rewriter.rs`.
...
`sqe-policy/src/session_udf.rs`. Each bakes in the session's
...
(`sqe-policy/src/tag_source.rs`; `NoopTagSource` by default,
