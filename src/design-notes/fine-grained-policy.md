...
- `sqe-policy/src/lib.rs`: `PolicyEnforcer`, `PolicyStore`,
...
- `sqe-policy/src/sha256_udf.rs`: the only registered policy mask UDF
  (HMAC-SHA256 for the Hash mask).
- `sqe-core/src/session.rs`: `SessionUser { username, roles: Vec<String> }`
...
**Phase 2A (shipped, branch `a feature branch`).** The full Ranger hive built-in mask
